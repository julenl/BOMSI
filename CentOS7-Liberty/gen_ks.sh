...
sed -i 's/menu title CentOS 7/menu title BOMSI installer for OpenStack Liberty on CentOS 7/' $TMPF
...
for HOST in $FULL_SERVER_LIST
 do 
  ...
  if [[ "$NAME_HOST" = "controller" ]]; then
     ...
     basic_net_ext $IFACE_EXT
     #basic_net_tunnel  \${IPPR_T}31 $IFACE2
   ...
   ## This is the "custom" script for each node

   echo "#! /bin/bash" > $OUT_PREF_PI/node_scripts/setup-$NAME_HOST
   echo "  trap "" 1  " >> $OUT_PREF_PI/node_scripts/setup-$NAME_HOST
   ...
   # The next lines are common in all nodes
   echo "  yum -y install http://dl.fedoraproject.org/pub/epel/7/x86_64/e/epel-release-7-5.noarch.rpm " >> $OUT_PREF_PI/node_scripts/setup-$NAME_HOST
   echo "  yum -y install centos-release-openstack-liberty " >> $OUT_PREF_PI/node_scripts/setup-$NAME_HOST
   echo "  yum -y update" >> $OUT_PREF_PI/node_scripts/setup-$NAME_HOST
   echo "  yum -y install openstack-selinux selinux-policy deltarpm yum-utils python-chardet python-kitchen ntp yum-plugin-priorities audit-libs-python checkpolicy libcgroup libsemanage-python policycoreutils-python python-IPy setools-libs net-tools nmap sshpass " >> $OUT_PREF_PI/node_scripts/setup-$NAME_HOST
   echo "   " >> $OUT_PREF_PI/node_scripts/setup-$NAME_HOST
   echo "   " >> $OUT_PREF_PI/node_scripts/setup-$NAME_HOST

   # The following line adds the specific functions to run on each node
...
case "\$1" in
  start)
     ...
     echo \$BOMSI_LIB_CONF_RELEASE \$BOMSI_LIB_CONF_VERSION "Step 1"|tee -a /root/kklog
     ...
     echo \$BOMSI_LIB_CONF_RELEASE \$BOMSI_LIB_CONF_VERSION "##########Step 2"|tee -a /root/kklog
     #yum -y update
     echo \$BOMSI_LIB_CONF_RELEASE \$BOMSI_LIB_CONF_VERSION "##########Step 3" |tee -a /root/kklog
     #yum -y upgrade
     ...
     echo \$BOMSI_LIB_CONF_RELEASE \$BOMSI_LIB_CONF_VERSION "##########Step 4" |tee -a /root/kklog
     log_step 'Common packages installed'
     log_step 'Starting node_script' 
 
     nohup /root/postinstall/node_scripts/setup-$NAME_HOST > /tmp/setup-$NAME_HOST.log &   
     echo \$BOMSI_LIB_CONF_RELEASE \$BOMSI_LIB_CONF_VERSION "##########Step 5" |tee -a /root/kklog
