...
## gather_packages (kilo)
# This script collects all the packages needed to install a full OpenStack Kilo environment
...
openstack-keystone python-openstackclient
httpd mod_wsgi memcached  python-memcached
openstack-glance python-glance python-glanceclient
openstack-nova-api openstack-nova-cert openstack-nova-conductor openstack-nova-console openstack-nova-novncproxy openstack-nova-scheduler python-novaclient 
...
sshpass -p $ROOT_PASSWORD ssh root@$IPTMP yum -y install --downloaddir /tmp/tmp-pkg/ https://repos.fedorapeople.org/repos/openstack/openstack-kilo/rdo-release-kilo-1.noarch.rpm
#sshpass -p 1234 ssh root@$IPMTP curl -o /tmp/tmp-pkg/rdo-release-kilo.rpm  http://rdo.fedorapeople.org/openstack-kilo/rdo-release-kilo.rpm
